...
class TerminalCommand():
    ...
    def run_terminal(self, dir, parameters):
        try:
            ...
            print "filename is ", self.window.active_view().file_name()
            ...
            for i in range( len( args ) ):
                if "$file" in args[ i ]:
                    args[ i ] = args[ i ].replace( "$file", filename )
                if "$config" in args[ i ]:
                    args[ i ] = args[ i ].replace( "$config", configDir )
            # Debug:
            print "new args are: ", " ".join(args)
...
class OpenTerminalCommand(sublime_plugin.WindowCommand, TerminalCommand):
    def run(self, paths=[], parameters=None):
        ...
        # If there were not parameters in settings, don't break
        if not parameters:
            parameters = []
